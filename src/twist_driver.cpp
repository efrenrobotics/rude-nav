...
namespace rude_nav {
    // we need to define our goal point. 
    geometry_msgs::Point Driver::getGoalPoint(geometry_msgs::Point base_point) {
        // return a different point
        // maybe a live feed of the mid-points being calculated by overhead kinect
    ...
    // find angular_difference between the base_transform and base_goal
    ...
    // conversion between a move_base from AMRL to a valid twist msg
    ...
    // get rid of hallway stuff, keep person detection
    bool Driver::step(){
        ...
        if(person_in_frame && base_to_person_dist < 8.5 && cooldown <= 0 && !finish_run){
            ...
            if(closest_lane_person == LANE_LEFT){
                _next_point = _hallway_lanes.closestOffsetPointLane3(base_point, _north_star, goal_offset);
		        goal_lane = LANE_3;
            }
